...
private:
     ...
     int	m_MaxNum;	//the max thread num that can create at the same time
     int	m_AvailLow;	//The min num of idle thread that shoule kept
     int	m_AvailHigh;	//The max num of idle thread that kept at the same time
     int	m_AvailNum;	//the normal thread num of idle num;
					//if the num of thread between m_AvailLow and m_AvailHign
					//we shouldn't change it
     int	m_InitNum;	//Normal thread num;
protected:
